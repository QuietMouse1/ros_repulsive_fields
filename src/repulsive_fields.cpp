...
void RepulsiveFields::run(){
    ...
    while(ros::ok()){
        ...
        sum_repulsive_force = Vector3f(0,0,0);
        int points = 0;
        ...
        for(int i = 0; i < pointcloud2_->size(); ++i){
            ...
            sum_repulsive_force += repulsive_force;
            ++points;
        }

        sensor_msgs::PointCloud2 msg;
        pcl::toROSMsg(*pointcloud2_, msg);
        msg.header.frame_id = "map";
        msg.header.stamp = ros::Time::now();
        repulsive_pcl_pub_.publish(msg); // Publish the filtered point cloud for debug

        if(points){ // normalise repulsive force
            sum_repulsive_force /= points;
        }

        sum_repulsive_force = k_repulsive * sum_repulsive_force;

        if (distance(sum_repulsive_force) > MAX_force){
            sum_repulsive_force = MAX_force * (sum_repulsive_force / distance(sum_repulsive_force)); // normalize to MAX_force
        }
        if (distance(sum_repulsive_force) < 0.1){
            sum_repulsive_force = Vector3f(0, 0, 0); 
        }

        ROS_INFO_STREAM("[RepulsiveFields] sum_repulsive_force = " << sum_repulsive_force);

        geometry_msgs::Vector3 debug_msg;
        debug_msg.x = sum_repulsive_force(0);
        debug_msg.y = sum_repulsive_force(1);
        debug_msg.z = 0;
        ...
        end.x = odometry_x + sum_repulsive_force(0);
        end.y = odometry_y + sum_repulsive_force(1);
        end.z = 0.0;
